...
voltList = [[5.0, "V", 1], [2.5, "V", 1], [1.0, "V", 1], [500, "mV", 0.001], [200, "mV", 0.001], [100, "mV", 0.001],
            [50, "mV", 0.001]]
timeList = [[50, "S", 1], [20, "S", 1], [10, "S", 1], [5, "S", 1], [2, "S", 1], [1, "S", 1], [500, "mS", .001],
            [200, "mS", .001], [100, "mS", .001], [50, "mS", .001], [20, "mS", .001], [10, "mS", .001], [5, "mS", .001],
            [2, "mS", .001], [1, "mS", .001], [500, "uS", 1E-6], [200, "uS", 1E-6], [100, "uS", 1E-6], [50, "uS", 1E-6],
            [20, "uS", 1E-6], [10, "uS", 1E-6], [5, "uS", 1E-6], [2, "uS", 1E-6], [1, "uS", 1E-6], [500, "nS", 1E-9],
            [200, "nS", 1E-9], [100, "nS", 1E-9], [50, "nS", 1E-9], [20, "nS", 1E-9], [10, "nS", 1E-9]]
...
jsObj = {
    "voltage": {"volts": [50, 50], "units": ["mV/div", "mv/div"], "multiplier": [0.001, 0.001], "probe": [1, 1],
                "coupling": ["DC", "DC"]},
    ...
    "measures": {"Vmax": [0, 0], "Vmin": [0, 0], "Vavg": [0, 0], "Vrms": [0, 0], "Vpp": [0, 0], "Vp": [0, 0],
                 "Freq": [0, 0], "Cycle": [0, 0], "Time+": [0, 0], "Time-": [0, 0], "Duty+": [0, 0], "Duty-": [0, 0]},
    "dataBuffer": [
        {"channel": "CH1", "units": "mV", "values": [0]*1500},
        {"channel": "CH2", "units": "mV", "values": [0]*1500},
    ],
    "dataScreen": [
        {"channel": "CH1", "units": "mV", "values": [0]*750},
        {"channel": "CH2", "units": "mV", "values": [0]*750},
    ],
}
...
def getBinaryData(filename):
    with open(filename, 'rb') as f:
        f.seek(0)
        header.append(f.read(208))
        f.seek(208)
        measures.append(f.read(48))
        f.seek(256)
        measures.append(f.read(48))
        f.seek(1000)
        dataBuff.append(f.read(3000))
        f.seek(4000)
        dataBuff.append(f.read(3000))
        f.seek(7000)
        dataScreen.append(f.read(1500))
        f.seek(8500)
        dataScreen.append(f.read(1500))
...
def parseData():
    for x in range(2):
        ...
        for y in range(0, 1500):
            jsObj["dataBuffer"][x]["values"][y] = (dataBuff[x][y * 2] + 256 * dataBuff[x][y * 2 + 1] - 200) * \
                                                  voltScale[x][0] / 50
